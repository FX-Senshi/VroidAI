...
const CUTE_VOICE_RATE = 0.88;
const CUTE_VOICE_PITCH = 1.92;
const SHARED_TTS_MODE = "shared-cute";
const SHARED_TTS_OPTION_LABEL = "自動 (共通TTSおすすめ)";
const BROWSER_VOICE_OPTION_LABEL = "ブラウザ音声 (端末依存)";
...
const VOICE_SETTINGS_STORAGE_KEY = "vroid-voice-settings-v3";
...
const MOBILE_MODEL_PITCH_OFFSET = 0;
...
const FIXED_MODEL_LITE_ASSET_FILE_NAME = "girlver2-lite.vrm";
const FIXED_MODEL_MOBILE_ASSET_FILE_NAME = "girlver2-mobile.vrm";
...
const FIXED_GIRL_FALLBACK_TIMING_SCALE = 0.78;
const FIXED_GIRL_FALLBACK_CHAR_DURATION_MS = 84;
const FIXED_GIRL_SPEECH_LEAD_MS = 104;
const FIXED_GIRL_SILENT_NEXT_BLEND = 0;
const FIXED_GIRL_SPEECH_DIRECT_LIMIT = 0.66;
const FIXED_GIRL_SPEECH_DROP_LIMIT = 0.3;
const FIXED_GIRL_SPEECH_LIFT_LIMIT = 0.09;
...
let voiceTools = null;
let availableVoices = [];
let latestApiStatus = null;
let lipSyncDebugPre = null;
const cameraToolRanges = new Map();
let speechSynthesisPrimed = false;
let speechPrimeListenerAttached = false;
let sharedTtsAudioContext = null;
let sharedTtsAudioGainNode = null;
let activeSharedTtsSource = null;
let activeSharedTtsAudioElement = null;
let activeSharedTtsObjectUrl = "";
...
function stopSharedTtsPlayback() {
  if (activeSharedTtsSource) {
    try {
      ...
      // Ignore stop failures from already-ended sources.
    }
    try {
  ...
  try {
    ...
    window.setTimeout(() => {
      try {
        ...
        // Ignore mobile browser speech cleanup failures.
      }
    }, 40);
  } catch {
...
function updateVoiceTools() {
  ...
  const selectedValue = isSharedTtsMode(settings)
    ? SHARED_TTS_MODE
    : settings.voiceURI
  ...
  if (isSharedTtsMode(settings)) {
    voiceTools.voiceStatus.textContent = supportsSharedTts()
      ? (MINIMAL_APP_MODE ? "共通TTS (かわいい声)" : "返答を共通TTSで読み上げます。")
      : (MINIMAL_APP_MODE ? "共通TTS未設定" : "共通TTS未設定です。API設定を確認してください。");
    return;
  }
...
function getCuteSpeechProfile(voice) {
  ...
  return {
    rate: topPriorityCute
      ? 0.84
      : japaneseVoice
        ? (likelyFemale ? 0.88 : 0.9)
        : CUTE_VOICE_RATE,
    pitch: topPriorityCute
      ? 2.02
      : japaneseVoice && likelyFemale
        ? 1.94
        : likelyFemale
        ? CUTE_VOICE_PITCH
        : 1.72
...
async function initViewer() {
  ...
  const maxPixelRatio = isNetlifyDeployment()
    ? shouldPreferLiteFixedModel()
      ? 2
      : 2.5
    : 2;
...
function getModelAssetFileName(fileName) {
  ...
  return shouldPreferLiteFixedModel()
    ? FIXED_MODEL_MOBILE_ASSET_FILE_NAME
    : FIXED_MODEL_NAME;
}
...
function normalizeLipSyncBlend(blend, deltaSec = speechState.lastApplyDeltaSec || 1 / 60) {
  const clampedBlend = THREE.MathUtils.clamp(blend, 0, 0.999);
  const frameScale = THREE.MathUtils.clamp((deltaSec || 1 / 60) * 60, 0.35, 2.4);
  ...
  const effectiveMouthActivity = Math.max(
    ...
    if (fixedGirlSpeechWindowActive) {
      ...
      const crossFadeToNext = currentSpeechFrame
        ? currentSpeechSilent
          ? THREE.MathUtils.smoothstep(currentSpeechProgress, 0.95, 1)
          : THREE.MathUtils.smootherstep(currentSpeechProgress, 0.04, 0.24)
        : 0;
      const currentPoseBlend = currentSpeechSilent
        ? 0
        : THREE.MathUtils.clamp(1 - crossFadeToNext * 0.98, 0.06, 1);
      const nextPoseBlend = currentSpeechSilent
        ? 0
        : THREE.MathUtils.clamp(Math.max(crossFadeToNext, THREE.MathUtils.smoothstep(currentSpeechProgress, 0.02, 0.18)), 0, 1);
      ...
      setDirectSupportMorphValue(
        "open",
        Math.max(
          supportTargets.open + fixedGirlOpenStrength * 0.008,
          fixedGirlSpeechIntensity > 0.012 ? 0.001 + currentPoseWeight * 0.006 : 0
        ),
        0.72,
        0.6
      );
      setDirectSupportMorphValue("drop", finalDropValue, 0.7, 0.6);
      setDirectSupportMorphValue("lift", finalLiftValue, 0.68, 0.58);
      setDirectSupportMorphValue("surprised", finalSurprisedValue, 0.62, 0.54);
  ...
  const pushPatternFrames = (char, pattern, strengthScale = 1, durationScale = 1) => {
    ...
    for (const part of normalizedPattern) {
      pushFrame({
        ...
        strength: THREE.MathUtils.clamp((Number(part.strength) || 0.4) * strengthScale, minStrength, maxStrength),
        utf16Start,
        utf16End
      });
...
function applyConversationMotion(vrmRef, elapsed, delta = 1 / 60) {
  ...
  const safeDelta = Math.max(delta || 0, 1 / 120);
  const weightDamping = speechState.active
    ? 13
    : targetWeight > currentWeight
      ? 8.2
      : 5.8;
  motionState.currentWeight = THREE.MathUtils.damp(
    currentWeight,
    targetWeight,
    weightDamping,
    safeDelta
  ...
  const thinkingDamping = effectiveThinkingTarget > (motionState.thinkingPoseWeight || 0)
    ? 5.4
    : 3.8;
  motionState.thinkingPoseWeight = THREE.MathUtils.damp(
    motionState.thinkingPoseWeight || 0,
...
function startLipSync(text, options = 1) {
  ...
  const fallbackTimingScale = (1 / normalizedRate) * (
  ...
  const durationTimingScale = expectedDurationMs > 0 && speechState.totalDurationMs > 0
    ? expectedDurationMs / speechState.totalDurationMs
    : 0;
  speechState.timingScale = durationTimingScale > 0
    ? durationTimingScale
    : fallbackTimingScale;
  speechState.timingOffsetMs = 0;
  speechState.leadMs = Number.isFinite(normalizedOptions.leadMs)
    ? Number(normalizedOptions.leadMs)
    : (currentModel === FIXED_MODEL_NAME ? FIXED_GIRL_SPEECH_LEAD_MS : 0);
...
function applyLipSync(vrmRef) {
  ...
  const nowMs = performance.now();
  const applyDeltaSec = speechState.lastApplyAtMs > 0
    ? THREE.MathUtils.clamp((nowMs - speechState.lastApplyAtMs) / 1000, 1 / 240, 1 / 18)
    : 1 / 60;
  ...
  if (speechState.active) {
    ...
    if (currentIndex === -1) {
      ...
      const crossFadeToNext = silentFrame
        ...
        : useFixedGirlDirectSpeech
          ? THREE.MathUtils.smootherstep(frameProgress, 0.06, 0.3)
          : THREE.MathUtils.smoothstep(frameProgress, 0.66, 0.94);
      const nextBlendWeight = useFixedGirlDirectSpeech
        ? Math.max(crossFadeToNext, THREE.MathUtils.smoothstep(frameProgress, 0.04, 0.22))
        : crossFadeToNext;
      const holdCurrent = silentFrame
        ? 0
        : useFixedGirlDirectSpeech
          ? THREE.MathUtils.clamp(1 - nextBlendWeight * 0.96, 0.08, 1)
          : 1 - crossFadeToNext * 0.22;
      ...
      if (useFixedGirlDirectSpeech && !silentFrame) {
        ...
        if (nextFrame?.viseme && !isSilentLipSyncFrame(nextFrame)) {
          fixedGirlLiveTargets[nextFrame.viseme] = Math.max(
            ...
            (nextFrame.strength || 0) * nextBlendWeight * (FIXED_GIRL_LIVE_FRAME_NEXT_FLOOR + 0.18)
          );
        }
      ...
      if (!silentFrame && previousFrame?.viseme && frameProgress < 0.08) {
        const carry = previousFrame.strength * (1 - frameProgress / 0.08) * (useFixedGirlDirectSpeech ? 0.008 : 0.05);
        ...
        if (nextFrame?.viseme) {
          targetValues[nextFrame.viseme] = Math.max(
            ...
            nextFrame.strength * nextBlendWeight * (
              silentFrame
    ...
    if (speechState.lastBoundaryFrameIndex >= 0) {
      ...
      if (sameVisemeBoundary && boundaryAgeMs < boundaryWindowMs) {
        const boundaryBoost = (1 - boundaryAgeMs / boundaryWindowMs)
          ...
          * (useFixedGirlDirectSpeech ? 0.04 : 0.08);
    ...
    for (const viseme of MOUTH_VISEMES) {
      ...
      const rawTargetValue = targetValues[viseme] || 0;
        ...
        const visemeBlendFactor = speechState.active
          ? silenceDecayMode
            ? (scaledTargetValue > currentValue ? (useFixedGirlDirectSpeech ? 0.3 : 0.12) : 0.42)
            : scaledTargetValue > currentValue
              ? (
                  useFixedGirlDirectSpeech
                    ? (speechState.boundarySupported ? 0.88 : 0.8)
                    : (speechState.boundarySupported ? 0.34 : 0.24)
                )
              : (useFixedGirlDirectSpeech ? 0.34 : 0.22)
          : 0.18;
...
async function playSharedTtsBlob(text, blob) {
  ...
  if (context) {
    try {
      ...
      startLipSync(text, {
        expectedDurationMs: audioBuffer.duration * 1000,
        leadMs: 0
      });
...
function speakWithBrowserSpeech(text, voiceSettings) {
  ...
  try {
    ...
    // Ignore resume failures on mobile browsers.
  }
  ...
  for (const delayMs of [0, 140, 420, 900]) {
    resumeTimerIds.push(window.setTimeout(() => {
      ...
      try {
        ...
        // Ignore mobile browser resume failures.
      }
    }, delayMs));
  }
  ...
  speechState.bootstrapWatcherId = window.setInterval(() => {
    ...
    if (useSystemVoice && waitedMs > 520 && waitedMs < 900) {
      try {
        ...
        // Ignore resume failures on iOS Safari.
      }
    }
    if (waitedMs > 2200) {
      beginLipSync();
